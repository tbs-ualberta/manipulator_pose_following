...
void cb_delta_pose_rpy(
    const manipulator_teleop::DeltaPoseRPY::ConstPtr &msg) {
  ...
  ROS_DEBUG_NAMED(
      ...
      g_delta_pose.delta_pose_rpy.at(0), g_delta_pose.delta_pose_rpy.at(1), g_delta_pose.delta_pose_rpy.at(2),
      g_delta_pose.delta_pose_rpy.at(3), g_delta_pose.delta_pose_rpy.at(4), g_delta_pose.delta_pose_rpy.at(5));
}
...
int main(int argc, char **argv) {
  ...
  //kinematic_state = boost::shared_ptr<moveit::core::RobotState>(new moveit::core::RobotState(kinematic_model));
  ...
  ROS_DEBUG("joint_names: [%s %s %s %s %s %s %s]",
  ...
  while (ros::ok()) {
    ...
    */
    ...
    {
      sum_squares = sum_squares + g_delta_pose.delta_pose_rpy.at(i)*g_delta_pose.delta_pose_rpy.at(i);
    }
    double delta_mag = pow(sum_squares, 0.5);
    for(unsigned int i=0; i<3; i++)
    {
      if(delta_mag > 0.0)
        vel_command[i] = g_delta_pose.delta_pose_rpy.at(i)/delta_mag*jogging_velocity*cart_translation_vel;
      else
        vel_command[i] = 0.0;
    }
    ...
    {
      if(delta_mag > 0.0)
        vel_command[i] = g_delta_pose.delta_pose_rpy.at(i)/delta_mag*jogging_velocity*cart_rotation_vel;
      else
        vel_command[i] = 0.0;
    }
    ...
    kinematic_state.getJacobian(joint_model_group,
       kinematic_state.getLinkModel(joint_model_group->getLinkModelNames().back()),ref_point, J);
    ...
    ROS_DEBUG_NAMED("stream_J_cnd", "Jacobian condition: %e", jacobian_condition);
