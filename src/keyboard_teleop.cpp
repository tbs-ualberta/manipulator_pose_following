...
int main(int argc, char **argv) {
  ...
  ros::AsyncSpinner spin_startstop(1, &queue_set_vel);
  ...
  ros::ServiceServer srv_set_velocity =
    nh_set_vel.advertiseService("/teleop/set_velocity", cb_set_vel);
  ...
  while (ros::ok && !stop) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
      if (delta_mag > 0.0)
        vel_command[i] = dof.data.at(i) / delta_mag *
